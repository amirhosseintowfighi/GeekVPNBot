...
from geekvpn.infrastructure.bot.checkout import _as_stored_id, _as_uuid
...
def test_an_id_survives_the_round_trip_the_receipt_flow_makes() -> None:
    ...
    assert _as_stored_id(_as_uuid(stored)) == stored
...
def test_str_would_not_have_worked() -> None:
    ...
    assert str(_as_uuid(stored)) != stored
...
def test_an_id_that_is_not_a_uuid_at_all_does_not_crash_the_handler() -> None:
    ...
    assert _as_uuid("not-an-id") == uuid.UUID(int=0)
