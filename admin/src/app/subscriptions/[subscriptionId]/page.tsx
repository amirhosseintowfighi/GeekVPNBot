      ...
      <Dialog open={action !== null} onOpenChange={(open) => (open ? null : close())}>
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              {action === 'suspend'
                ? 'اتصال قطع می‌شه ولی اکانت روی پنل می‌مونه و بعداً برمی‌گردونیش.'
                : action === 'revoke'
                  ? 'اکانت از روی پنل حذف می‌شه. این کار برگشت‌پذیر نیست.'
                  : action === 'extend'
                    ? 'روزهای اضافه بدون گرفتن پول به اشتراک اضافه می‌شه.'
                    : 'حجم اضافه بدون گرفتن پول به سقف اشتراک اضافه می‌شه.'}
            </DialogDescription>
          </DialogHeader>
