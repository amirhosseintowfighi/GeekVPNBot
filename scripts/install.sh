...
# Which compose files this deployment is made of.
#
# Docker Compose reads this, so a bare `docker compose ...` in this directory
# targets the stack that is actually deployed. Without it the base file alone is
# used, and every command naming a production-only service - worker, api_blue,
# api_green, admin, miniapp, certbot - answers "no such service", which reads
# like the service has died rather than like the wrong file was loaded.
#
# scripts/deploy.sh and the Makefile pass -f explicitly and are unaffected.
...
TELEGRAM__MINI_APP_URL=https://app.${DOMAIN}
